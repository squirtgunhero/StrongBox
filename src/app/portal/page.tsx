  ...
  if (error) {
    return (
      <div className="rounded-lg border bg-white p-12 text-center">
        <p className="text-sm text-stone-500">Unable to load portal data</p>
      </div>
    );
  }
  ...
  return (
    <div>
      <div className="mb-6">
        ...
        <p className="text-sm text-stone-500 mt-1">Investor Dashboard</p>
      </div>
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-6">
        <div className="rounded-lg border bg-white p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-xs text-stone-500">Account Balance</p>
        ...
        <div className="rounded-lg border bg-white p-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-xs text-stone-500">Interest Earned</p>
      ...
      <div className="rounded-lg border bg-white">
        <div className="flex items-center justify-between px-4 py-3 border-b">
          ...
          <Link href="/portal/portfolio" className="text-xs text-[#1E3A5F] hover:text-[#162D4A]">
            View All
          </Link>
        </div>
        {portfolio.length === 0 ? (
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b bg-stone-50">
                <th className="px-4 py-2.5 text-left font-medium text-stone-500">Loan</th>
                <th className="px-4 py-2.5 text-left font-medium text-stone-500">Property</th>
                <th className="px-4 py-2.5 text-right font-medium text-stone-500">Investment</th>
                <th className="px-4 py-2.5 text-right font-medium text-stone-500">Rate</th>
                <th className="px-4 py-2.5 text-left font-medium text-stone-500">Status</th>
              </tr>
            </thead>
            <tbody>
              {portfolio.slice(0, 10).map((inv: any) => (
                <tr key={inv.loanNumber} className="border-b last:border-0">
                  <td className="px-4 py-2.5 font-medium">{inv.loanNumber}</td>
                  <td className="px-4 py-2.5 text-xs text-stone-500">{inv.property}</td>
      ...
      {data?.transactions && data.transactions.length > 0 && (
        <div className="mt-6 rounded-lg border bg-white">
          <div className="flex items-center justify-between px-4 py-3 border-b">
            ...
            <Link href="/portal/transactions" className="text-xs text-[#1E3A5F] hover:text-[#162D4A]">
              View All
            </Link>
          </div>
            ...
            {data.transactions.slice(0, 5).map((tx: any) => (
              <div key={tx.id} className="flex items-center justify-between px-4 py-3">
                <div>
                  ...
                  <p className="text-xs text-stone-500">{tx.description || "—"}</p>
                </div>
                <div className="text-right">
                  <p className={cn("text-sm font-medium", tx.amount > 0 ? "text-emerald-600" : "text-red-600")}>
                    {tx.amount > 0 ? "+" : ""}{formatCurrency(tx.amount)}
                  </p>
                  <p className="text-[10px] text-stone-400">{new Date(tx.date).toLocaleDateString()}</p>
                </div>
              </div>
            ))}
          </div>
