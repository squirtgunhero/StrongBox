    ...
    <div className="rounded-lg border bg-white p-5 dark:bg-zinc-900 dark:border-zinc-800">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          ...
          <h3 className="text-sm font-semibold">Maturity Calendar</h3>
        </div>
      ...
      <div className="grid grid-cols-4 gap-2 mb-4">
        {[
          ...
          { label: "< 60d", data: maturityBuckets.within60, color: "text-brand-600" },
          { label: "< 90d", data: maturityBuckets.within90, color: "text-zinc-600" },
        ].map((b) => (
          <div key={b.label} className="text-center">
            <p className={cn("text-lg font-bold", b.data.count > 0 ? b.color : "text-zinc-300")}>{b.data.count}</p>
      ...
      {upcomingMaturities.length === 0 ? (
        <p className="text-xs text-zinc-400 text-center py-4">No upcoming maturities</p>
      ) : (
            ...
            return (
              <div
                ...
                className={cn(
                  ...
                  isPast && "bg-red-50/50 dark:bg-red-950/20"
                )}
              >
                <div className="flex items-center gap-2">
                  {isPast && <AlertTriangle className="h-3 w-3 text-red-500" />}
                  <span className="font-medium">{loan.loanNumber}</span>
                  <span className="text-zinc-400">{loan.borrower}</span>
                </div>
                <span className={cn("font-medium", isPast ? "text-red-600" : isUrgent ? "text-amber-600" : "")}>
