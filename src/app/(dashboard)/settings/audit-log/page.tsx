...
const ACTION_COLORS: Record<string, string> = {
  ...
  UPDATE: "bg-brand-100 text-brand-700 dark:bg-brand-950 dark:text-brand-400",
    ...
    <div>
      ...
      <div className="flex gap-3 mb-4">
        <select
          ...
          className="h-9 rounded-md border bg-white px-3 text-sm dark:bg-zinc-800 dark:border-zinc-700"
        >
              ...
              {data.logs.map((log: any) => (
                <div key={log.id}>
                  <button
                    ...
                    className="w-full flex items-center gap-4 px-4 py-3 text-left hover:bg-zinc-50 dark:hover:bg-zinc-800/30 transition-colors"
                  >
                    <span
                      className={cn(
                    ...
                    <span className="text-xs text-zinc-400 min-w-[130px] text-right">
                  ...
                  {expandedId === log.id && log.diff && (
                    <div className="px-4 pb-3">
                      <div className="rounded-md bg-zinc-50 dark:bg-zinc-800 p-3 text-xs font-mono overflow-x-auto">
                            ...
                            {Object.entries(log.diff as Record<string, any>).map(
                              ([field, change]: [string, any]) => (
                                <tr key={field}>
                                  ...
                                  <td className="pr-4 py-0.5 text-red-600 dark:text-red-400">
                                    {JSON.stringify(change.from)}
                                  </td>
                                  <td className="py-0.5 text-green-600 dark:text-green-400">
