...
export default function LoanDetailPage() {
  ...
  if (error || !data?.loan) {
    return (
      <div className="p-12 text-center">
        ...
        <Link href="/loans" className="text-sm text-brand-600 mt-2 inline-block">
          Back to loans
        </Link>
      </div>
    );
  ...
  return (
    <div>
      ...
      {/* Tabs */}
      <div className="border-b mb-6 dark:border-zinc-800">
...
function StatusMenu({
  ...
  return (
    <>
      ...
      <div className="absolute left-0 top-full mt-1 z-50 w-48 rounded-md border bg-white shadow-lg dark:bg-zinc-900 dark:border-zinc-800">
        {options.map((status) => (
          <button
            key={status}
      ...
      <div className="lg:col-span-2 space-y-6">
        ...
        <div className="rounded-lg border bg-white p-6 dark:bg-zinc-900 dark:border-zinc-800">
          ...
          <dl className="grid grid-cols-2 gap-3 text-sm">
            ...
            <div><dt className="text-zinc-500">Exit Strategy</dt><dd className="font-medium">{loan.exitStrategy?.replace(/_/g, " ") || "-"}</dd></div>
            <div><dt className="text-zinc-500">Loan Amount</dt><dd className="font-medium">{formatCurrency(loan.loanAmount)}</dd></div>
            <div><dt className="text-zinc-500">Current Balance</dt><dd className="font-medium">{formatCurrency(loan.currentBalance)}</dd></div>
            <div><dt className="text-zinc-500">Interest Rate</dt><dd className="font-medium">{Number(loan.interestRate).toFixed(2)}%</dd></div>
            <div><dt className="text-zinc-500">Term</dt><dd className="font-medium">{loan.termMonths} months</dd></div>
            {loan.purchasePrice && <div><dt className="text-zinc-500">Purchase Price</dt><dd className="font-medium">{formatCurrency(loan.purchasePrice)}</dd></div>}
            {loan.rehabBudget && <div><dt className="text-zinc-500">Rehab Budget</dt><dd className="font-medium">{formatCurrency(loan.rehabBudget)}</dd></div>}
            {loan.ltv && <div><dt className="text-zinc-500">LTV</dt><dd className="font-medium">{Number(loan.ltv).toFixed(1)}%</dd></div>}
          </dl>
        </div>
        ...
        {loan.property && (
          <div className="rounded-lg border bg-white p-6 dark:bg-zinc-900 dark:border-zinc-800">
            ...
            <dl className="grid grid-cols-2 gap-3 text-sm">
              ...
              <div><dt className="text-zinc-500">Type</dt><dd className="font-medium">{loan.property.propertyType}</dd></div>
              {loan.property.yearBuilt && <div><dt className="text-zinc-500">Year Built</dt><dd className="font-medium">{loan.property.yearBuilt}</dd></div>}
              {loan.property.squareFeet && <div><dt className="text-zinc-500">Sq Ft</dt><dd className="font-medium">{loan.property.squareFeet.toLocaleString()}</dd></div>}
              {loan.property.bedrooms && <div><dt className="text-zinc-500">Beds/Baths</dt><dd className="font-medium">{loan.property.bedrooms}/{loan.property.bathrooms}</dd></div>}
            </dl>
          </div>
        )}
      </div>
      ...
      <div className="space-y-6">
        <div className="rounded-lg border bg-white p-6 dark:bg-zinc-900 dark:border-zinc-800">
          ...
          <dl className="space-y-2 text-sm">
            ...
            {loan.approvalDate && <div className="flex justify-between"><dt className="text-zinc-500">Approved</dt><dd>{formatDate(loan.approvalDate)}</dd></div>}
            {loan.fundingDate && <div className="flex justify-between"><dt className="text-zinc-500">Funded</dt><dd>{formatDate(loan.fundingDate)}</dd></div>}
            {loan.maturityDate && <div className="flex justify-between"><dt className="text-zinc-500">Maturity</dt><dd>{formatDate(loan.maturityDate)}</dd></div>}
            <div className="flex justify-between"><dt className="text-zinc-500">Created</dt><dd>{formatDate(loan.createdAt)}</dd></div>
          </dl>
        </div>
        ...
        <div className="rounded-lg border bg-white p-6 dark:bg-zinc-900 dark:border-zinc-800">
          ...
          <dl className="space-y-2 text-sm">
            ...
            {loan.processor && <div className="flex justify-between"><dt className="text-zinc-500">Processor</dt><dd>{loan.processor.firstName} {loan.processor.lastName}</dd></div>}
            {loan.underwriter && <div className="flex justify-between"><dt className="text-zinc-500">Underwriter</dt><dd>{loan.underwriter.firstName} {loan.underwriter.lastName}</dd></div>}
          </dl>
        </div>
      ...
      <table className="w-full text-sm">
        ...
        <tbody>
          {documents.map((doc: any) => (
            <tr key={doc.id} className="border-b last:border-0 dark:border-zinc-800">
              <td className="px-4 py-3 font-medium">{doc.fileName}</td>
              <td className="px-4 py-3 text-zinc-500">{doc.category}</td>
              <td className="px-4 py-3">
                <span className={cn("text-xs rounded-full px-2 py-0.5", doc.reviewStatus === "ACCEPTED" ? "bg-green-100 text-green-700" : "bg-zinc-100 text-zinc-600")}>
                  {doc.reviewStatus || "Pending"}
                </span>
              </td>
              <td className="px-4 py-3 text-zinc-500 text-xs">{formatDate(doc.createdAt)}</td>
            </tr>
          ))}
        </tbody>
      </table>
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b bg-zinc-50 dark:bg-zinc-800/50 dark:border-zinc-800">
            ...
            <th className="px-4 py-3 text-left font-medium text-zinc-500">Principal</th>
            <th className="px-4 py-3 text-left font-medium text-zinc-500">Interest</th>
            <th className="px-4 py-3 text-left font-medium text-zinc-500">Status</th>
            <th className="px-4 py-3 text-left font-medium text-zinc-500">Paid</th>
          </tr>
        </thead>
        <tbody>
          {payments.map((pmt: any) => (
            <tr key={pmt.id} className="border-b last:border-0 dark:border-zinc-800">
              ...
              <td className="px-4 py-3 text-zinc-500">{formatCurrency(pmt.principalAmount)}</td>
              <td className="px-4 py-3 text-zinc-500">{formatCurrency(pmt.interestAmount)}</td>
              <td className="px-4 py-3">
                <span className={cn("text-xs rounded-full px-2 py-0.5", {
                  ...
                  "bg-zinc-100 text-zinc-600": pmt.status === "SCHEDULED",
                })}>
                  {pmt.status}
                </span>
              </td>
              <td className="px-4 py-3 text-zinc-500 text-xs">{pmt.paidDate ? formatDate(pmt.paidDate) : "-"}</td>
            </tr>
          ))}
        </tbody>
      </table>
      ...
      <table className="w-full text-sm">
        <thead>
          <tr className="border-b bg-zinc-50 dark:bg-zinc-800/50 dark:border-zinc-800">
            ...
            <th className="px-4 py-3 text-left font-medium text-zinc-500">Requested</th>
            <th className="px-4 py-3 text-left font-medium text-zinc-500">Approved</th>
            <th className="px-4 py-3 text-left font-medium text-zinc-500">Status</th>
        ...
        <tbody>
          {draws.map((draw: any) => (
            <tr key={draw.id} className="border-b last:border-0 dark:border-zinc-800">
              ...
              <td className="px-4 py-3">
                <span className="text-xs rounded-full px-2 py-0.5 bg-zinc-100 text-zinc-600">{draw.status}</span>
              </td>
    ...
    <div className="space-y-2">
      {conditions.map((cond: any) => (
        <div
          key={cond.id}
          className={cn(
            "rounded-lg border bg-white p-4 flex items-start gap-3 dark:bg-zinc-900 dark:border-zinc-800",
        ...
        >
          ...
          <div>
            ...
            {cond.category && <p className="text-xs text-zinc-400 mt-1">{cond.category}</p>}
      ...
      {history.map((entry: any) => (
        <div key={entry.id} className="flex items-start gap-3">
          ...
          <div>
            ...
            {entry.reason && <p className="text-xs text-zinc-500 mt-1">{entry.reason}</p>}
            <p className="text-xs text-zinc-400 mt-1">{formatRelative(entry.createdAt)}</p>
          </div>
        </div>
      ))}
    </div>
