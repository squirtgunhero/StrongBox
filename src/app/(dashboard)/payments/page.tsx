...
const STATUS_CONFIG: Record<string, { label: string; color: string; bg: string }> = {
  SCHEDULED: { label: "Scheduled", color: "text-brand-600", bg: "bg-brand-50 dark:bg-brand-950" },
  PENDING: { label: "Pending", color: "text-amber-600", bg: "bg-amber-50 dark:bg-amber-950" },
  PAID: { label: "Paid", color: "text-emerald-600", bg: "bg-emerald-50 dark:bg-emerald-950" },
  LATE: { label: "Late", color: "text-red-600", bg: "bg-red-50 dark:bg-red-950" },
  NSF: { label: "NSF", color: "text-red-700", bg: "bg-red-100 dark:bg-red-950" },
  WAIVED: { label: "Waived", color: "text-zinc-500", bg: "bg-zinc-100 dark:bg-zinc-800" },
  PARTIAL: { label: "Partial", color: "text-amber-600", bg: "bg-amber-50 dark:bg-amber-950" },
};
    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-zinc-500 mt-1">{total} total payments</p>
        </div>
      </div>
        ...
        <div className="rounded-lg border bg-white p-4 dark:bg-zinc-900 dark:border-zinc-800">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-zinc-500">Due in 30 days</p>
            </div>
          </div>
        </div>
        <div className="rounded-lg border bg-white p-4 dark:bg-zinc-900 dark:border-zinc-800">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-zinc-500">Paid this month</p>
            </div>
          </div>
        </div>
      </div>
          ...
          <button
            ...
            className={cn(
              ...
              statusFilter === tab.value
                ? "bg-brand-50 text-brand-700 border-brand-200 dark:bg-brand-950 dark:text-brand-400 dark:border-brand-800"
                : "text-zinc-500 border-zinc-200 hover:bg-zinc-50 dark:border-zinc-700 dark:hover:bg-zinc-800"
            )}
          >
            {tab.label}
          </button>
      ...
      {isLoading ? (
        ...
        <div className="rounded-lg border bg-white p-12 text-center dark:bg-zinc-900 dark:border-zinc-800">
          <DollarSign className="h-10 w-10 text-zinc-300 mx-auto mb-3" />
              ...
              {payments.map((payment: any) => {
                ...
                return (
                  <tr
                    key={payment.id}
                    className={cn(
                      "border-b last:border-0 dark:border-zinc-800 hover:bg-zinc-50 dark:hover:bg-zinc-800/30",
