    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-stone-500 mt-1">Generate and export lending reports</p>
        </div>
        <button
          onClick={handleExportCsv}
          disabled={isExporting || isLoading}
          className="flex items-center gap-2 rounded-md border px-4 py-2 text-sm font-medium hover:bg-stone-50 disabled:opacity-50"
        >
      ...
      <div className="grid grid-cols-5 gap-3 mb-6">
        {REPORT_TYPES.map((rt) => {
          ...
          return (
            ...
            >
              <Icon className={cn("h-4 w-4 mb-1.5", reportType === rt.value ? "text-[#1E3A5F]" : "text-stone-400")} />
              <p className={cn("text-sm font-medium", reportType === rt.value ? "text-[#162D4A]" : "")}>{rt.label}</p>
              <p className="text-[10px] text-stone-500 mt-0.5">{rt.description}</p>
            </button>
          );
        })}
      </div>
      ...
      {isLoading ? (
        ...
        <div className="rounded-lg border bg-white p-12 text-center">
          ...
          <p className="text-sm text-stone-500">Select a report type to generate</p>
        </div>
      ) : (
        <>
          {/* Summary Cards */}
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b bg-stone-50">
              ...
              <th className="px-4 py-3 text-left font-medium text-stone-500">Property</th>
              ...
              <th className="px-4 py-3 text-left font-medium text-stone-500">Officer</th>
              <th className="px-4 py-3 text-right font-medium text-stone-500">Days</th>
            </tr>
          </thead>
          <tbody>
            {report.loans.map((l: any) => (
              <tr key={l.loanNumber} className="border-b last:border-0">
                <td className="px-4 py-3 font-medium text-[#1E3A5F]">{l.loanNumber}</td>
                <td className="px-4 py-3">{l.borrower}</td>
                <td className="px-4 py-3 text-xs text-stone-500">{l.property}</td>
                <td className="px-4 py-3 text-right font-medium">{formatCurrency(l.loanAmount)}</td>
                <td className="px-4 py-3">
                  <span className="inline-flex items-center rounded-full bg-[#EFF4F9] px-2 py-0.5 text-[10px] font-medium text-[#1E3A5F]">
                    {l.status.replace("_", " ")}
                  </span>
                </td>
                <td className="px-4 py-3 text-xs text-stone-500">{l.loanOfficer}</td>
                <td className="px-4 py-3 text-right text-xs text-stone-500">{l.daysInPipeline}</td>
              </tr>
            ))}
          </tbody>
        </table>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b bg-stone-50">
              ...
              <th className="px-4 py-3 text-right font-medium text-stone-500">Rate</th>
              <th className="px-4 py-3 text-right font-medium text-stone-500">Maturity</th>
              <th className="px-4 py-3 text-right font-medium text-stone-500">Days Left</th>
            ...
            {report.loans.map((l: any) => (
              <tr key={l.loanNumber} className={cn("border-b last:border-0", l.daysToMaturity !== null && l.daysToMaturity < 0 && "bg-red-50/50950/20")}>
                ...
                <td className="px-4 py-3 text-right">
                  <span className={cn("text-xs font-medium", l.daysToMaturity < 0 ? "text-red-600" : l.daysToMaturity <= 30 ? "text-amber-600" : "text-stone-500")}>
                    {l.daysToMaturity ?? "—"}
                  </span>
                </td>
                <td className="px-4 py-3 text-right text-xs text-stone-500">{l.ltv ? `${l.ltv.toFixed(1)}%` : "—"}</td>
              </tr>
            ))}
          </tbody>
        </table>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b bg-stone-50">
              ...
              <th className="px-4 py-3 text-left font-medium text-stone-500">Borrower</th>
              <th className="px-4 py-3 text-right font-medium text-stone-500">Balance</th>
              <th className="px-4 py-3 text-right font-medium text-stone-500">Days Late</th>
              <th className="px-4 py-3 text-left font-medium text-stone-500">Bucket</th>
              <th className="px-4 py-3 text-right font-medium text-stone-500">NSFs</th>
    ...
    case "investor":
      return (
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b bg-stone-50">
              <th className="px-4 py-3 text-left font-medium text-stone-500">Source</th>
              <th className="px-4 py-3 text-left font-medium text-stone-500">Type</th>
              <th className="px-4 py-3 text-left font-medium text-stone-500">Bank</th>
              <th className="px-4 py-3 text-right font-medium text-stone-500">Limit</th>
              <th className="px-4 py-3 text-right font-medium text-stone-500">Deployed</th>
              <th className="px-4 py-3 text-right font-medium text-stone-500">Available</th>
              <th className="px-4 py-3 text-right font-medium text-stone-500">Util %</th>
              <th className="px-4 py-3 text-right font-medium text-stone-500">Loans</th>
            </tr>
          </thead>
          <tbody>
            {report.sources.map((s: any) => (
              <tr key={s.name} className="border-b last:border-0">
                ...
                <td className="px-4 py-3 text-xs text-stone-500">{s.bankName}</td>
                ...
                <td className="px-4 py-3 text-right text-xs text-stone-500">{s.allocations.length}</td>
              </tr>
            ))}
          </tbody>
        </table>
    ...
    case "revenue":
      return (
        <table className="w-full text-sm">
          <thead>
            <tr className="border-b bg-stone-50">
              <th className="px-4 py-3 text-left font-medium text-stone-500">Month</th>
              <th className="px-4 py-3 text-right font-medium text-stone-500">Interest</th>
              <th className="px-4 py-3 text-right font-medium text-stone-500">Fees</th>
              <th className="px-4 py-3 text-right font-medium text-stone-500">Principal</th>
              <th className="px-4 py-3 text-right font-medium text-stone-500">Total Revenue</th>
            </tr>
          </thead>
          <tbody>
            {report.monthlyBreakdown.map((m: any) => (
              <tr key={m.month} className="border-b last:border-0">
                ...
                <td className="px-4 py-3 text-right text-stone-500">{formatCurrency(m.principal)}</td>
                <td className="px-4 py-3 text-right font-medium text-emerald-600">{formatCurrency(m.total)}</td>
              </tr>
            ))}
            {report.monthlyBreakdown.length === 0 && (
              <tr>
                <td colSpan={5} className="px-4 py-12 text-center text-stone-400 text-sm">
