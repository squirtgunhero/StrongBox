      ...
      <div className="grid grid-cols-5 gap-3 mb-6">
        {ANALYTICS_TYPES.map((at) => {
          ...
          return (
            ...
            >
              <Icon className={cn("h-4 w-4 mb-1.5", type === at.value ? "text-brand-600" : "text-zinc-400")} />
    ...
    <div className="rounded-lg border bg-white dark:bg-zinc-900 dark:border-zinc-800">
      <table className="w-full text-sm">
        ...
        <tbody>
          ...
          {officers.length === 0 && (
            <tr><td colSpan={7} className="px-4 py-12 text-center text-zinc-400">No loan officers found</td></tr>
          )}
        </tbody>
      </table>
    </div>
            ...
            {(data.byState || []).map((s: any) => (
              <div key={s.state}>
                ...
                <div className="w-full bg-zinc-100 dark:bg-zinc-800 rounded-full h-2">
                  <div
                    className={cn("h-2 rounded-full transition-all", s.concentration > 50 ? "bg-red-500" : s.concentration > 25 ? "bg-amber-500" : "bg-brand-500")}
            ...
            {(data.byCity || []).slice(0, 10).map((c: any) => (
              <div key={`${c.city}-${c.state}`} className="flex items-center justify-between text-xs">
                ...
                <div className="text-right">
                  ...
                  <span className="text-zinc-400 ml-2">({c.concentration.toFixed(1)}%)</span>
                </div>
              </div>
            ))}
          </div>
    ...
    <div className="space-y-6">
      <div className="grid grid-cols-3 gap-4">
        {[
          ...
          <div key={c.label} className="rounded-lg border bg-white p-4 dark:bg-zinc-900 dark:border-zinc-800">
      ...
      <div className="rounded-lg border bg-white dark:bg-zinc-900 dark:border-zinc-800">
        <table className="w-full text-sm">
          ...
          <tbody>
            {byType.map((t: any) => (
              <tr key={t.type} className="border-b last:border-0 dark:border-zinc-800">
...
function DefaultProbability({ data }: { data: any }) {
  ...
  const riskColors: Record<string, string> = {
    ...
    LOW: "text-brand-600 bg-brand-50 dark:bg-brand-950/20",
    MINIMAL: "text-emerald-600 bg-emerald-50 dark:bg-emerald-950/20",
  };
            ...
            {riskByLTV.map((r: any) => (
              <div key={r.label} className="flex items-center justify-between text-xs">
                ...
                <div className="text-right">
                  ...
                  <span className="text-zinc-400 ml-2">({r.count} loans, {r.defaults} defaults)</span>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="space-y-2">
            {riskByDelinquency.map((r: any) => (
              <div key={r.label} className="flex items-center justify-between text-xs">
                ...
                <div className="text-right">
                  <span className={cn(r.rate > 10 ? "text-red-600" : "")}>{r.rate.toFixed(1)}%</span>
                  <span className="text-zinc-400 ml-2">({r.count} loans)</span>
                </div>
              </div>
            ))}
          </div>
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {scoredLoans.slice(0, 30).map((loan: any) => (
              <tr key={loan.loanNumber} className="border-b last:border-0 dark:border-zinc-800">
                ...
                <td className="px-4 py-3 text-[10px] text-zinc-400 max-w-[200px] truncate">{loan.factors.join(", ")}</td>
              </tr>
            ))}
            {scoredLoans.length === 0 && (
              <tr><td colSpan={8} className="px-4 py-12 text-center text-zinc-400">No active loans to score</td></tr>
            )}
          </tbody>
        </table>
      </div>
