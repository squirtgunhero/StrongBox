          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b bg-stone-50">
                ...
                <th className="px-4 py-3 text-right font-medium text-stone-500">Requested</th>
                <th className="px-4 py-3 text-right font-medium text-stone-500">Approved</th>
            ...
            <tbody>
              {allDraws.map((draw: any) => (
                <tr key={draw.id} className="border-b last:border-0">
                  ...
                  <td className="px-4 py-3 text-stone-500">{draw.loanNumber}</td>
...
function DrawStatusBadge({ status }: { status: string }) {
  const config: Record<string, { icon: typeof CheckCircle2; color: string }> = {
    ...
    UNDER_REVIEW: { icon: Clock, color: "text-[#1E3A5F] bg-[#EFF4F9]" },
  ...
  return (
    <div className="rounded-lg border bg-white p-5 mb-6">
      ...
      <div className="grid grid-cols-2 gap-4 mb-4">
        <div>
          <label className="block text-xs font-medium text-stone-500 mb-1">Loan</label>
          <select
            value={loanId}
