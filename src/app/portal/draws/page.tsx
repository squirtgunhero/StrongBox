    ...
    <div>
      ...
      {isLoading ? (
        ...
        <div className="rounded-lg border bg-white p-12 text-center dark:bg-zinc-900 dark:border-zinc-800">
          <Hammer className="h-10 w-10 text-zinc-300 mx-auto mb-3" />
...
function DrawStatusBadge({ status }: { status: string }) {
  const config: Record<string, { icon: typeof CheckCircle2; color: string }> = {
    SUBMITTED: { icon: Clock, color: "text-amber-600 bg-amber-50 dark:bg-amber-950/20" },
    UNDER_REVIEW: { icon: Clock, color: "text-brand-600 bg-brand-50 dark:bg-brand-950/20" },
    APPROVED: { icon: CheckCircle2, color: "text-emerald-600 bg-emerald-50 dark:bg-emerald-950/20" },
    FUNDED: { icon: CheckCircle2, color: "text-emerald-600 bg-emerald-50 dark:bg-emerald-950/20" },
    REJECTED: { icon: XCircle, color: "text-red-600 bg-red-50 dark:bg-red-950/20" },
  };
