...
export default function ContactDetailPage() {
  ...
  if (error || !data?.contact) {
    return (
      <div className="p-12 text-center">
        ...
        <Link href="/contacts" className="text-sm text-brand-600 mt-2 inline-block">
          Back to contacts
        </Link>
      </div>
    );
  ...
  return (
    <div>
      <Link
        href="/contacts"
        className="inline-flex items-center gap-1 text-sm text-zinc-500 hover:text-zinc-700 mb-4 dark:hover:text-zinc-300"
      >
      ...
      <div className="flex items-start justify-between mb-6">
        ...
        {contact.naughtyLevel > 0 && (
          <div className="flex items-center gap-1 rounded-md bg-red-50 border border-red-200 px-3 py-1.5 text-sm text-red-700 dark:bg-red-950 dark:border-red-900 dark:text-red-400">
  ...
  return (
    <div className="rounded-lg border bg-white dark:bg-zinc-900 dark:border-zinc-800">
      <table className="w-full text-sm">
        ...
        <tbody>
          {loans.map((loan: any) => (
            <tr key={loan.id} className="border-b last:border-0 dark:border-zinc-800">
      ...
      {communications.map((comm: any) => (
        ...
        >
          <div className="flex items-center justify-between mb-2">
            <span className="text-xs font-medium rounded-full bg-zinc-100 px-2 py-0.5 dark:bg-zinc-800">
              {comm.type.replace(/_/g, " ")}
            </span>
            <span className="text-xs text-zinc-400">{formatDate(comm.createdAt)}</span>
          </div>
          {comm.subject && <p className="text-sm font-medium">{comm.subject}</p>}
          <p className="text-sm text-zinc-600 dark:text-zinc-400 mt-1">{comm.body}</p>
        </div>
      ))}
    </div>
  );
