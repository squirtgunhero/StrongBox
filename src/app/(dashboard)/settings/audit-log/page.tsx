...
const ACTION_COLORS: Record<string, string> = {
  ...
  UPDATE: "bg-[#EFF4F9] text-[#162D4A]",
  DELETE: "bg-red-100 text-red-700400",
      ...
      <div className="rounded-lg border bg-white">
        {isLoading ? (
          ...
          <div className="p-12 text-center">
            <ScrollText className="h-10 w-10 text-stone-300 mx-auto mb-3" />
            <p className="text-sm text-stone-500">No audit logs found</p>
          </div>
        ) : (
          <>
                ...
                <div key={log.id}>
                  ...
                  >
                    <span
                      className={cn(
                        ...
                        ACTION_COLORS[log.action] || "bg-stone-100 text-stone-700"
                      )}
                    >
                      {log.action}
                    </span>
                    <span className="text-xs text-stone-400 font-medium min-w-[60px]">
                      {log.entityType}
                    </span>
                    ...
                    <span className="text-xs text-stone-400 min-w-[130px] text-right">
                  ...
                  {expandedId === log.id && log.diff && (
                    <div className="px-4 pb-3">
                      <div className="rounded-md bg-stone-50 p-3 text-xs font-mono overflow-x-auto">
                          ...
                          <tbody>
                            {Object.entries(log.diff as Record<string, any>).map(
                              ([field, change]: [string, any]) => (
                                <tr key={field}>
                                  <td className="pr-4 py-0.5 text-stone-500">
                                    {field}
                                  </td>
